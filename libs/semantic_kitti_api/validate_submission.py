...
parser = argparse.ArgumentParser(description="Validate a submission zip file needed to evaluate on CodaLab competitions."
                                             "\n\nThe verification tool checks:\n  1. correct folder structure,\n  "
                                             "2. existence of label files for each scan,\n  "
                                             "3. count of labels for each scan.\nInvalid labels are ignored by the "
                                             "evaluation script, therefore we don't check\nfor invalid labels.",
                                 formatter_class=argparse.RawTextHelpFormatter)

parser.add_argument("--zipfile",
                    type=str,
                    default="Datasets/SemanticKitti/dataset/predictions_sequences_laserid_range_images_2048/sequences.zip",
                    help='zip file that should be validated.')

parser.add_argument('--dataset',
                    type=str,
                    default="Datasets/SemanticKitti/dataset/",
                    help='directory containing the folder "sequences" containing folders "11", ..., "21" with the input data '
                         '("velodyne" or "voxels") folder.')
